...
string LoggerSingleton::LogLevels[] = {JFR_LOGGER_LEVEL_FATAL, \
										JFR_LOGGER_LEVEL_ERROR, \
										JFR_LOGGER_LEVEL_WARNING, \
										JFR_LOGGER_LEVEL_INFO, \
										JFR_LOGGER_LEVEL_DEBUG_1, \
										JFR_LOGGER_LEVEL_DEBUG_2, \
										JFR_LOGGER_LEVEL_DEBUG_3};
...
int LoggerSingleton::Init(int nBaseLevel, const string& sFilePath, const string& sLogPrefix, bool bIsDev)
{
    ...
    if (nBaseLevel < 0 || (size_t)nBaseLevel >= sizeof(LoggerSingleton::LogLevels) / sizeof(string*))
	{
		...
		ss2 << nBaseLevel << endl;
		cerr << MODULE_JFR"[ERROR]: log base level error, valid base level is [0 ~ " << ss1.str() << "]set base level is " << ss2.str() << endl;
		return -1;
	}
	...
	if (ret != 0)
	{
		cerr << MODULE_JFR"[ERROR]: log init failed. " << endl;
		SHOW_ERROR(err);
		return -1;
	}
